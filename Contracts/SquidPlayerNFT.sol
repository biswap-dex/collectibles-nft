    ...
    ReentrancyGuardUpgradeable
{
    ...
    function setMintLockTime(uint _mintLockTimeDuration, uint _mintLockStartTime)
        external
    ...
    function getSEAmountFromTokensId(uint[] calldata _tokenId)
        external
        view
        returns (uint totalSeAmount, uint[] memory tokenSeAmount)
    {
        totalSeAmount = 0;
    ...
    function squidEnergyDecrease(
        uint[] calldata tokenId,
        uint128[] calldata deduction,
        address user
    ) public onlyRole(SE_BOOST_ROLE) {
    ...
    function squidEnergyIncrease(
        uint[] calldata tokenId,
        uint128[] calldata addition,
        address user
    ) public onlyRole(SE_BOOST_ROLE) {
    ...
    function _beforeTokenTransfer(
        ...
        if (_tokens[tokenId].createTimestamp > mintLockStartTime && from != address(0) && to != address(0))
            require(
                (block.timestamp - _tokens[tokenId].createTimestamp) >= mintLockDuration,
                "mint lock time not ended"
            );
    ...
    function _lockToken(
        ...
        bool willDecrease,
        uint contractVersion
    ) private returns (uint128 SEAmount, uint128 currentSE) {
        require(_exists(_tokenId), "ERC721: token does not exist");
    ...
    function _setPlayerContract(uint _tokenId, uint32 _contractDuration)
        private
        returns (uint32 _contractEndTimestamp)
    {
        Token storage _token = _tokens[_tokenId];
        require(!_token.stakeFreeze, "Token frozen");
    ...
    function _setPlayerContractV2(uint _tokenId, uint32 _contractDuration)
        private
        returns (uint32 _contractEndTimestamp)
    {
