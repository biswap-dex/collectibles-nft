...
contract Market is ReentrancyGuard, Ownable, Pausable {
    ...
    constructor(address _treasuryAddress, ISwapFeeRewardWithRB _feeRewardRB) {
        //NFT-01
    ...
    function setRewardDistributionSeller(uint256 _rewardDistributionSeller) public onlyOwner {
    ...
    function offer(
        ...
        uint256 tokenId,
        uint256 price
    ) public nonReentrant whenNotPaused _nftAllowed(nft) _validDealToken(dealToken) notContract {
        if (side == Side.Buy) {
    ...
    function cancel(uint256 id) public nonReentrant _offerExists(id) _offerOpen(id) _offerOwner(id) whenNotPaused {
        if (offers[id].side == Side.Buy) {
            _cancelBuy(id);
        } else {
            _cancelSell(id);
